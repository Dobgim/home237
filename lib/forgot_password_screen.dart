  ...
  Future<void> _sendResetLink() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        setState(() => _isLoading = false);
        if (mounted) {
          final errorMessage = AppLocalizations.of(context).getAuthErrorMessage(e.code, e.message);
        ...
        setState(() => _isLoading = false);
        if (mounted) {
          final errorMessage = AppLocalizations.of(context).get('err_unexpected');
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(errorMessage),
              backgroundColor: Colors.red,
  ...
  void _showSuccessDialog(String email) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Column(
          children: [
            Icon(Icons.mark_email_read, color: Color(0xFF1E3A5F), size: 48),
            SizedBox(height: 16),
