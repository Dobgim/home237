    ...
    if (userId != null) {
      ...
      try {
        ...
        if (verSnap.docs.isNotEmpty) {
          ...
          verStatus = (docs.first.data()['status'] ?? 'pending') as String;
        }
      } catch (e) {
      ...
      if (verStatus != 'approved') {
        ...
        await showDialog(
          ...
          builder: (ctx) {
            ...
            if (verStatus == 'pending') {
              dialogTitle = 'Verification Pending';
              dialogBody =
                  'Your documents have been submitted and are currently under review by our admin team. '
                  'You will be able to post properties once your account is approved. '
                  'Please check back later.';
              accentColor = const Color(0xFFF59E0B);
              dialogIcon = Icons.hourglass_top_rounded;
            } else if (verStatus == 'rejected') {
              dialogTitle = 'Verification Rejected';
              dialogBody =
                  'Your verification was rejected. Please re-upload your documents '
                  'and wait for admin approval before posting properties.';
              accentColor = const Color(0xFFEF4444);
              dialogIcon = Icons.gpp_bad_outlined;
            } else {
              dialogTitle = 'Verification Required';
              dialogBody =
              ...
              content: Column(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(dialogIcon, size: 56, color: accentColor),
                  ),
                  Padding(
              ...
              actions: [
                ...
                if (verStatus == 'none' || verStatus == 'rejected')
                  ElevatedButton.icon(
                    ...
                    label: Text(verStatus == 'rejected' ? 'Re-upload Docs' : 'Get Verified'),
                    onPressed: () {
                      Navigator.pop(ctx);
                      Navigator.push(
                        context,
        ...
        return; // Stop here — do not proceed to AddPropertyScreen
      }
    }

    // ─── Step 2: Premium & Limits Gate ─────────────────────────────────────────
    if (userId != null) {
      try {
        final propertiesSnapshot = await FirebaseFirestore.instance
            .collection('properties')
        ...
        print('Error checking listing limits: $e');
      }
    }

    // ─── Step 3: Navigate to Add Property ────────────────────────────────────
