...
class _EmailVerificationScreenState extends State<EmailVerificationScreen> {
  ...
  /// Polls Firebase every 4 seconds to detect when the user clicks the
  /// verification link in their email, then auto-routes them to the dashboard.
  void _startAutoPoll() {
    _pollTimer?.cancel();
    _pollTimer = Timer.periodic(const Duration(seconds: 4), (_) async {
      if (_autoVerified || !mounted) return;
      try {
        final user = FirebaseAuth.instance.currentUser;
        if (user == null) return;
        await user.reload();
        ...
        if (refreshed != null && refreshed.emailVerified) {
          ...
          // Show success banner then navigate
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: const Row(children: [
                ...
                SizedBox(width: 10),
                Expanded(child: Text(
                  '✅ Email verified! Routing you to your dashboard…',
          ...
          if (mounted) {
            await _completeSignup(refreshed.uid);
          }
        }
      } catch (_) {}
    });
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          tooltip: t.get('back') ?? 'Back',
          onPressed: () async {
            // If the user wants to change their account/email, sign them out and return to Home
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const Expanded(
                      child: Text(
                        'Waiting for you to click the link in your email… This page will update automatically.',
                        style: TextStyle(
                          fontSize: 13,
